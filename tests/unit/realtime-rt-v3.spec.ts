...
describe('Realtime RT V3', () => {
  ...
  it('ignores retired and intentionally destroyed WebGPU devices before Beauty recovery', () => {
    ...
    expect(recoverySource).toContain("if (reasonCode === 'destroyed')");
    expect(recoverySource).toContain('webgpuIgnoredDestroyedDeviceLoss');
    expect(recoverySource).toContain('webgpuDestroyedDeviceMessage');
    const identityGate = recoverySource.indexOf('viewer.renderer?.backend?.device !== device');
    const destroyedGate = recoverySource.indexOf("if (reasonCode === 'destroyed')");
    ...
    expect(recoverySource).not.toContain('/^device was destroyed');
