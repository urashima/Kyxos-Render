...
test('Project Chat sends, replies, edits, moderates and persists across two sessions', async ({ page }) => {
  ...
  await editor.locator('.project-chat-message').filter({ hasText: 'Hello Editor' }).getByRole('button', { name: 'Reply' }).click();
  ...
  await editorMessage.getByRole('button', { name: 'Edit' }).click();
  ...
  await owner.locator('.project-chat-message').filter({ hasText: 'Updated reply' }).getByRole('button', { name: 'Delete' }).click();
...
test('Project Chat publishes typing state and keeps user actions scoped', async ({ page }) => {
  ...
  await expect(editorMessage.getByRole('button', { name: 'Edit' })).toHaveCount(0);
  await expect(editorMessage.getByRole('button', { name: 'Delete' })).toHaveCount(1);

  const editorOwnMessage = editor.locator('.project-chat-message').filter({ hasText: 'Typing now' });
  await expect(editorOwnMessage.getByRole('button', { name: 'Edit' })).toHaveCount(1);
  await expect(editorOwnMessage.getByRole('button', { name: 'Delete' })).toHaveCount(1);
});
