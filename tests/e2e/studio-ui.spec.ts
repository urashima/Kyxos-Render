...
test('switches Authoring and Focus modes without replacing the viewport', async ({ page }) => {
  ...
  await page.evaluate(() => {
    (window as typeof window & { __kyxosCanvas?: HTMLCanvasElement }).__kyxosCanvas =
      document.querySelector<HTMLCanvasElement>('#studio-canvas') ?? undefined;
  });
  ...
  expect(
    await page.evaluate(
      () =>
        (window as typeof window & { __kyxosCanvas?: HTMLCanvasElement }).__kyxosCanvas ===
        document.querySelector('#studio-canvas'),
    ),
  ).toBe(true);
  ...
  await expect(page.locator('.kyxos-studio-shell')).toHaveAttribute(
    'data-kx-theme',
    'graphite',
  );
