...
import { RealtimeHybridRtFeaturePass } from './render/advanced/realtimeHybridRtFeaturePass';
...
const states = new WeakMap<any, RealtimeHybridRtController>();
const installKey = Symbol.for('kyxos.viewer.realtime-hybrid-rt');
...
class RealtimeHybridRtController {
  ...
  setMode(mode: 'realtime' | 'cinematic' | 'pathTracing'): void {
    ...
    if (mode === 'cinematic') {
      this.active = true;
      // The legacy advanced controller must stay realtime in Hybrid RT mode so
      // its opaque full-frame overlay can never replace the authoritative image.
  ...
  private async activate(reason: string): Promise<void> {
    ...
    if (!this.featurePass) {
      ...
      this.initialization = (async () => {
        this.setState('initializing', 'Initializing same-device Hybrid RT feature passes.');
        const pass = new RealtimeHybridRtFeaturePass(this.viewer, this.settings);
        try {
    ...
    if (!this.sceneDirty && this.featurePass) {
      ...
      this.viewer.queuePipelineRebuild?.(`hybrid-rt:${reason}`);
      this.setState('rendering', 'Realtime primary + same-device RT visibility/reflection + temporal filtering.');
    }
  }
  ...
  private async rebuildScene(): Promise<void> {
    ...
    this.setState('building', 'Building Hybrid RT software BVH on the realtime renderer device.');
  ...
  markSceneDirty(): void {
    ...
    if (this.active) void this.rebuildScene().then(() => {
      if (this.active && !this.sceneDirty) this.viewer.queuePipelineRebuild?.('hybrid-rt-scene-change');
    });
  }
  ...
  resetHistory(reason = 'manual'): void {
    this.featurePass?.resetHistory();
    this.readyUniform.value = 0;
    if (this.active) this.viewer.queuePipelineRebuild?.(`hybrid-rt-history:${reason}`);
  }
  ...
  private renderFeatureFrame(): void {
    ...
    if (this.viewer.animationEnabled) {
      this.readyUniform.value = 0;
      this.message = 'Hybrid RT pauses while skin/morph animation is changing the software BVH pose.';
      return;
    }
    ...
    try {
      ...
      if (frame.ready) {
        ...
        if (this.runtimeState !== 'rendering') this.setState('rendering', 'Hybrid RT temporally filtered inside the realtime pipeline.');
      }
    } catch (error) {
      this.readyUniform.value = 0;
  ...
  decoratePipeline(): void {
    ...
    visibilityTemporal.maxFrames.value = 24;
    ...
    reflectionTemporal.maxFrames.value = 24;
    ...
    const reflectionWeight = material.r.mul(0.75).add(0.04).mul(material.g.oneMinus()).mul(ready);
    ...
    this.viewer.canvas.dataset.advancedRenderArchitecture = 'realtime-hybrid-feature-pass';
  }
  ...
  getStatus(): any {
    ...
    const enabledFeatures = ['environmentImportance', 'softwareRayQuery'];
    ...
    // In Hybrid RT, the ReSTIR control selects multiple direct-light visibility
    // candidates but never adds a second direct-light estimate on top of Beauty.
    // This intentionally prevents the former double-energy exposure bug.
  ...
  private deactivateHybrid(): void {
    ...
    this.viewer.queuePipelineRebuild?.('hybrid-rt-disabled');
  }
...
export function installRealtimeHybridRtExtension(ViewerClass: { prototype: any }): void {
  ...
  const state = (viewer: any): RealtimeHybridRtController => {
    let value = states.get(viewer);
    if (!value) {
      value = new RealtimeHybridRtController(viewer, methods);
