...
const BUFFER_USAGE = (globalThis as any).GPUBufferUsage ?? {
  COPY_SRC: 4,
  COPY_DST: 8,
  UNIFORM: 64,
  STORAGE: 128,
...
export class WebGpuHybridRenderer {
  ...
  private sceneBuffers: SceneBuffers | null = null;
  private pixelBuffers: PixelBuffers | null = null;
  private computeBindGroups: any[] = [];
  private displayBindGroups: any[] = [];
  private scene: ExtractedAdvancedScene | null = null;
  private settings = normalizeAdvancedRenderSettings(null);
  ...
  async initialize(): Promise<AdvancedRendererCapabilities> {
    ...
    this.computePipeline = this.device.createComputePipeline({
      label: 'Kyxos.Advanced.PathPipeline',
      layout: 'auto',
      compute: { module: computeModule, entryPoint: 'main' },
    });
    this.displayPipeline = this.device.createRenderPipeline({
      label: 'Kyxos.Advanced.DisplayPipeline',
      layout: 'auto',
  ...
  setSettings(value: SceneAdvancedRenderSettings): void {
    ...
    this.settings = next;
    if (
      old.restirDI.mode !== next.restirDI.mode ||
      old.restirDI.candidates !== next.restirDI.candidates ||
      old.restirDI.spatialSamples !== next.restirDI.spatialSamples ||
      old.radianceCache.enabled !== next.radianceCache.enabled ||
      old.radianceCache.cellSize !== next.radianceCache.cellSize ||
      old.pathTracing.maxBounces !== next.pathTracing.maxBounces ||
      old.pathTracing.resolutionScale !== next.pathTracing.resolutionScale
    ) {
      this.resetAccumulation();
    }
    if (this.scene && old.radianceCache.capacity !== next.radianceCache.capacity) {
      this.uploadScene(this.scene);
    }
  }
  ...
  private uploadScene(scene: ExtractedAdvancedScene): void {
    ...
    this.sceneBuffers = {
      triangles: this.createStorage(scene.triangles, 'Kyxos.Advanced.Triangles'),
      nodes: this.createStorage(scene.nodes, 'Kyxos.Advanced.BVH'),
  ...
  private destroySceneBuffers(): void {
    ...
    for (const key of ['triangles', 'nodes', 'materials', 'lights', 'lightAlias', 'environmentPixels', 'environmentAlias', 'cache'] as const) {
      destroyBuffer(this.sceneBuffers[key]);
    }
  ...
  private destroyPixelBuffers(): void {
    ...
    for (const key of ['accumulation', 'reservoirA', 'reservoirB', 'surfaceA', 'surfaceB'] as const) {
      destroyBuffer(this.pixelBuffers[key]);
    }
  ...
  private ensurePixelBuffers(width: number, height: number): void {
    if (!this.device) return;
    if (this.pixelBuffers?.width === width && this.pixelBuffers.height === height) return;
    ...
    this.pixelBuffers = {
      ...
      width,
      height,
      bytes: accumulationBytes + reservoirBytes * 2 + surfaceBytes * 2,
    };
    this.rebuildBindGroups();
    this.resetAccumulation();
  }
  ...
  resetAccumulation(): void {
    ...
    if (this.pixelBuffers) {
      encoder.clearBuffer(this.pixelBuffers.accumulation);
      encoder.clearBuffer(this.pixelBuffers.reservoirA);
      encoder.clearBuffer(this.pixelBuffers.reservoirB);
      encoder.clearBuffer(this.pixelBuffers.surfaceA);
      encoder.clearBuffer(this.pixelBuffers.surfaceB);
    }
    ...
    const makeCompute = (previousReservoir: any, nextReservoir: any, previousSurface: any, nextSurface: any) =>
      this.device.createBindGroup({
        ...
        entries: [
          ...
          { binding: 1, resource: { buffer: s.triangles } },
          { binding: 2, resource: { buffer: s.nodes } },
          { binding: 3, resource: { buffer: s.materials } },
          { binding: 4, resource: { buffer: s.lights } },
          { binding: 5, resource: { buffer: s.lightAlias } },
          { binding: 6, resource: { buffer: p.accumulation } },
          { binding: 7, resource: { buffer: previousReservoir } },
          { binding: 8, resource: { buffer: nextReservoir } },
          { binding: 9, resource: { buffer: previousSurface } },
          { binding: 10, resource: { buffer: nextSurface } },
          { binding: 11, resource: { buffer: s.environmentPixels } },
          { binding: 12, resource: { buffer: s.environmentAlias } },
          { binding: 13, resource: { buffer: s.cache } },
        ],
      });
  ...
  private alignOverlay(mode: 'cinematic' | 'pathTracing'): void {
    ...
    this.overlay.style.width = `${Math.max(1, rect.width)}px`;
    this.overlay.style.height = `${Math.max(1, rect.height)}px`;
    this.overlay.style.borderRadius = getComputedStyle(this.baseCanvas).borderRadius;
    const dpr = Math.min(2, window.devicePixelRatio || 1);
    const configuredScale = this.settings.pathTracing.resolutionScale;
    const scale = mode === 'cinematic' ? Math.min(0.5, configuredScale) : configuredScale;
    let width = Math.max(1, Math.floor(rect.width * dpr * scale));
    let height = Math.max(1, Math.floor(rect.height * dpr * scale));
    const maxDimension = mode === 'cinematic' ? 1280 : 1920;
    const downscale = Math.min(1, maxDimension / Math.max(width, height));
    ...
    if (this.overlay.width !== width || this.overlay.height !== height || this.lastScale !== scale) {
      ...
      this.ensurePixelBuffers(width, height);
    }
  }

  private cameraData(camera: any): { data: Float32Array; signature: string } {
    camera.updateMatrixWorld?.(true);
    const elements = camera.matrixWorld?.elements as number[] | undefined;
    const position = camera.position?.clone?.() ?? { x: 0, y: 0, z: 5 };
    camera.getWorldPosition?.(position);
    const forward = { x: 0, y: 0, z: -1 } as any;
    camera.getWorldDirection?.(forward);
    const right = elements ? { x: elements[0], y: elements[1], z: elements[2] } : { x: 1, y: 0, z: 0 };
    const up = elements ? { x: elements[4], y: elements[5], z: elements[6] } : { x: 0, y: 1, z: 0 };
    const aspect = this.pixelBuffers ? this.pixelBuffers.width / Math.max(1, this.pixelBuffers.height) : 1;
    const tanHalfFov = Math.tan(((Number(camera.fov) || 50) * Math.PI) / 360);
    const materialCount = Math.max(1, this.scene?.materialCount ?? 1);
    const mode = this.settings.renderingMode;
    const bounces = mode === 'cinematic' ? Math.min(3, this.settings.pathTracing.maxBounces) : this.settings.pathTracing.maxBounces;
    const restir = this.settings.restirDI;
    const data = new Float32Array(36);
    data.set([position.x, position.y, position.z, tanHalfFov], 0);
    data.set([forward.x, forward.y, forward.z, aspect], 4);
    data.set([right.x, right.y, right.z, this.frameIndex], 8);
    data.set([up.x, up.y, up.z, camera.isOrthographicCamera ? 1 : 0], 12);
    data.set([
      this.pixelBuffers?.width ?? 1,
      this.pixelBuffers?.height ?? 1,
      bounces,
      this.scene?.lightCount ?? 0,
    ], 16);
    data.set([
      this.scene?.triangleCount ?? 0,
      this.scene?.bvh.nodes.length ?? 0,
      this.scene?.environmentWidth ?? 1,
      this.scene?.environmentHeight ?? 1,
    ], 20);
    data.set([
      restirModeNumber(restir.mode),
      restir.candidates,
      restir.spatialSamples,
      this.settings.radianceCache.enabled ? 1 : 0,
    ], 24);
    data.set([
      1,
      this.settings.pathTracing.denoise ? 1 : 0,
      this.settings.pathTracing.fireflyClamp,
      this.settings.pathTracing.samplesPerFrame,
    ], 28);
    data.set([
      this.settings.radianceCache.cellSize,
      this.sceneBuffers?.cacheCapacity ?? this.settings.radianceCache.capacity,
      this.settings.radianceCache.updateRatio,
      materialCount,
    ], 32);
    const signature = [
  ...
  render(camera: any, mode: 'cinematic' | 'pathTracing'): AdvancedGpuMetrics | null {
    ...
    const cameraData = this.cameraData(camera);
    if (this.lastCameraSignature && cameraData.signature !== this.lastCameraSignature) this.resetAccumulation();
    this.lastCameraSignature = cameraData.signature;
    // resetAccumulation rewinds frameIndex, so update frame-dependent data after the camera cut decision.
    const refreshed = this.cameraData(camera);
    ...
    try {
      ...
      compute.setBindGroup(0, this.computeBindGroups[this.ping]);
      compute.dispatchWorkgroups(Math.ceil(this.pixelBuffers.width / 8), Math.ceil(this.pixelBuffers.height / 8));
      compute.end();

      const target = this.context.getCurrentTexture().createView();
      const render = encoder.beginRenderPass({
        label: 'Kyxos.Advanced.Resolve',
        colorAttachments: [{
          view: target,
          loadOp: 'clear',
          storeOp: 'store',
      ...
      render.setPipeline(this.displayPipeline);
      render.setBindGroup(0, this.displayBindGroups[this.ping]);
      render.draw(3);
      render.end();
      this.device.queue.submit([encoder.finish()]);
      this.frameIndex += 1;
      this.samples += this.settings.pathTracing.samplesPerFrame;
  ...
  getMetrics(): AdvancedGpuMetrics {
    const historyBytes = this.pixelBuffers?.bytes ?? 0;
    const sceneBytes = this.scene?.estimatedBytes ?? 0;
    const cacheBytes = (this.sceneBuffers?.cacheCapacity ?? 0) * 20;
    return {
      width: this.pixelBuffers?.width ?? 0,
      height: this.pixelBuffers?.height ?? 0,
      samples: this.samples,
      cpuFrameTimeMs: this.lastCpuFrameTimeMs,
      sceneBytes,
      historyBytes,
      cacheBytes,
      totalBytes: sceneBytes + historyBytes + cacheBytes,
    };
  }

  async capture(mimeType = 'image/png', quality = 0.92): Promise<Blob> {
    return canvasBlob(this.overlay, mimeType, quality);
  }
