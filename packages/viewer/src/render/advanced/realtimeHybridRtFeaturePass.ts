...
import { realtimeHybridRtWGSL } from './realtimeHybridRtShader';
...
function compilationErrors(info: any): string | null {
  const errors = (info?.messages ?? []).filter((entry: any) => entry?.type === 'error');
  if (!errors.length) return null;
  return errors.slice(0, 8).map((entry: any) => {
...
/**
 * Runs software-ray feature queries on the GPUDevice already owned by
 * KyxosViewer's WebGPURenderer. It never creates a second adapter, device,
 * canvas or full-frame renderer: realtime remains the authoritative image.
 */
export class RealtimeHybridRtFeaturePass {
  ...
  constructor(viewer: any, settings: SceneAdvancedRenderSettings) {
    ...
    this.visibilityTexture.name = 'Kyxos.HybridRT.Visibility';
    ...
    this.reflectionTexture.name = 'Kyxos.HybridRT.Reflection';
  ...
  async initialize(): Promise<void> {
    ...
    const module = this.device.createShaderModule({
      label: 'Kyxos.HybridRT.FeatureCompute',
      code: realtimeHybridRtWGSL,
    });
    if (typeof module.getCompilationInfo === 'function') {
      const message = compilationErrors(await module.getCompilationInfo());
      if (message) throw new Error(`Hybrid RT WGSL compilation failed: ${message}`);
    }

    const layout = this.device.createBindGroupLayout({
      label: 'Kyxos.HybridRT.FeatureLayout',
      entries: [
    ...
    const pipelineLayout = this.device.createPipelineLayout({
      label: 'Kyxos.HybridRT.FeaturePipelineLayout',
    ...
    try {
      this.pipeline = typeof this.device.createComputePipelineAsync === 'function'
        ? await this.device.createComputePipelineAsync({
            label: 'Kyxos.HybridRT.FeaturePipeline',
    ...
    if (thrown || validationError) {
      const detail = thrown instanceof Error
        ? thrown.message
        : String((validationError as any)?.message ?? thrown ?? validationError ?? 'unknown validation error');
      throw new Error(`Hybrid RT pipeline validation failed: ${detail}`);
    }

    this.globalsBuffer = this.device.createBuffer({
      label: 'Kyxos.HybridRT.Globals',
  ...
  private uploadScene(scene: ExtractedAdvancedScene): void {
    ...
    this.staticSceneBuffer = this.createStorage(staticPool.data, 'Kyxos.HybridRT.StaticScene');
    this.dynamicSceneBuffer = this.createStorage(dynamicPool.data, 'Kyxos.HybridRT.DynamicScene');
    this.bindGroup = null;
    this.boundInputs = null;
  }

  private ensureOutputs(width: number, height: number): void {
    if (this.width === width && this.height === height && this.rawTexture(this.visibilityTexture) && this.rawTexture(this.reflectionTexture)) {
      return;
    }
  ...
  private ensureBindGroup(inputs: HybridRtFrameInputs): boolean {
    ...
    this.bindGroup = this.device.createBindGroup({
      label: 'Kyxos.HybridRT.FeatureBindGroup',
  ...
  private writeGlobals(camera: any): void {
    ...
    data.set([this.width, this.height, this.frameIndex, 0], 32);
    data.set([
  ...
  render(camera: any, inputs: HybridRtFrameInputs): HybridRtFrameState {
    if (this.disposed || !this.initialized || !this.scene) {
      return { ready: false, frameIndex: this.frameIndex, cpuFrameTimeMs: this.cpuFrameTimeMs };
    }
    ...
    const encoder = this.device.createCommandEncoder({ label: 'Kyxos.HybridRT.Frame' });
    const compute = encoder.beginComputePass({ label: 'Kyxos.HybridRT.FeatureQueries' });
    ...
    return { ready: true, frameIndex: this.frameIndex, cpuFrameTimeMs: this.cpuFrameTimeMs };
  }
