...
api = Path('packages/api-client/src/index.ts')
text = api.read_text()
text = replace_once(
    text,
...
old = "    globalThis.document.documentElement.dataset.durableDraftRevision = String(persistedRevision);\n    return persistedRevision;\n  }\n\n  let previewMode = false;"
new = """    globalThis.document.documentElement.dataset.durableDraftRevision = String(persistedRevision);
    return persistedRevision;
  }
  ...
  let previewMode = false;"""
text = replace_once(text, old, new, 'Studio live recovery helper')
text = replace_once(
    text,
...
text = spec.read_text()
if 'publish re-persists an imported Blob from the live manifest' not in text:
    text += r'''
